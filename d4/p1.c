...
int main (int argc, char *argv[])
{
    ...
    // GAME
    int won_number = -1;
    int won_board_index = -1;
    for (size_t i = 0; i < count && won_board_index < 0; i++)
    {
    ...
    print_marked_board(boards[won_board_index], marked[won_board_index]);   

    int result = 0;
    for (size_t i = 0; i < BOARD_SIZE; i++)
    {
        if (!(marked[won_board_index] & 1 << (BOARD_SIZE - i - 1))) {
            result += boards[won_board_index][i];
        }
    }
